...
-   **BonusPreviewState**: Pinia-owned structure that powers drag-to-highlight previews.
...
2.  **Game Engine Logic**: The core implementation for the new mechanics will reside in the Phaser game engine code under `src/game/engine/`.
    ...
    *   **Bonuses**: `BonusActivator.js` powers both activations and previews; extend it via the new `preview` helper before wiring animations.
...
3.  **UI Interaction**: The Vue components in `src/components/` will use the Pinia stores (`src/stores/`) to manage game state and interact with the game engine.
    ...
    *   **Bonus Drag Preview**:
        - `PowerUpBar.vue` captures pointer events and invokes `gameStore.startBonusDrag`.
        - `gameStore` computes `BonusPreviewState` via `BonusActivator.preview` and exposes `affectedIndices`.
        - `BoardAnimator` consumes `affectedIndices` via `showBonusPreview(indices, glowConfig)` to render the glowing blue tiles.
...
-   Add Vitest coverage for `BonusActivator.preview` (verify shape for each bonus type) and component tests that ensure `PowerUpBar` emits the drag lifecycle events.
