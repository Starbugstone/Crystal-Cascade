...
Prospect Hollow is a frontier town-building puzzle game built with Vue, Pinia, Phaser and Three.js. Match jewels through 120 levels across twenty chapters, earn resources, build a small 3D Old West settlement, and carry the town into new eras.
...
Complete all Frontier buildings to advance to **1884 · River & Rail Boom**. Modernize familiar landmarks while preserving their services, build a wharf and railway station, and open four east-bank plots by finishing the bridge. The station’s first construction includes its connecting railroad. The blacksmith makes one TNT after 20 completed normal puzzles at level 1, improving to 16, 12, 8 and 5 puzzles at levels 2–5. It stores only one; tap to collect it into your armory and start the next cycle. Mine earnings scale from 1× in chapter 1 to 20× in chapter 20. Coin chests pay 500 coins per chapter. Building upgrades have no completed-puzzle locks, and the level-4 town square gains a bell that halves remaining raid losses once per raid. Complete all three River & Rail levels on every plot to enter **1908 · Industrial / Electric Town**. Build a power house, fire station, row houses and mill, and give every existing landmark three substantial Industrial upgrades. Electricity lights the village; cargo theft and workshop fires give the later eras their own village encounters. Three eras are playable; later eras remain unavailable. Existing v3 progress is preserved. See [settlement eras and quick testing](docs/settlement-eras.md), including disposable demo saves generated with `npm run demo:eras`.
