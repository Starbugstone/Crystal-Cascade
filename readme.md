...
Complete all Frontier buildings and the River Discovery chapter to advance to **1884 · River & Rail Boom**. Modernize familiar landmarks while preserving their services, build a wharf and railway station, and open four east-bank plots by finishing the bridge. The station’s first construction includes its connecting railroad. The blacksmith makes one TNT after 20 completed normal puzzles at level 1, improving to 16, 12, 8 and 5 puzzles at levels 2–5. It stores only one; tap to collect it into your armory and start the next cycle. Bonus chest coins scale from 25 in chapter 1 to 300 in chapter 12. Only these first two eras are playable. Existing v3 progress is preserved. See [settlement eras and quick testing](docs/settlement-eras.md), including disposable demo saves generated with `npm run demo:eras`.
