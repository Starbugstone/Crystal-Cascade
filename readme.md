...
Complete all Frontier buildings and the River Discovery chapter to advance to **1884 · River & Rail Boom**. Modernize familiar landmarks while preserving their services, build a wharf and railway station, and open four east-bank plots by finishing the bridge. The station’s first construction includes its connecting railroad. The blacksmith stores one Forge Charge after five normal completions, redeemable before a run for one temporary puzzle Hammer. Only these first two eras are playable. Existing v3 progress is preserved. See [settlement eras and quick testing](docs/settlement-eras.md), including disposable demo saves generated with `npm run demo:eras`.
