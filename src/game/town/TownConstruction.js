...
export class TownConstruction {
  constructor(view, group, rotor) {
    ...
    for (const piece of this.pieces) piece.delay = clamp(piece.y / height) * 0.42;
    group.userData.static = false;
    group.userData.animated = true;
    if (rotor) rotor.visible = false;

    this.effects = view.group(group);
