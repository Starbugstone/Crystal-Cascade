...
- Saloon base income is now 2.25 coins per person per level per hour, with 1.25% extra income per happiness point. Its light green tag shows stored income; tap to collect with a coin burst and chimes. A later tap opens its details. Parcel-list selections open the card without collecting.
...
Chapter mining payouts now multiply the subtotal by the chapter number: 1× in chapter 1 through 12× in chapter 12. The recap shows the additional coins separately. Normal replay uses the level being replayed; continuous mode applies the same percentage to its gem income while retaining the 25-coin lifetime cap. Coin chests now pay 500 coins per chapter; saloon income also receives stronger population and happiness bonuses. Regression checks cover chapter boundaries, rounding, wallet/recap agreement, repeated settlement, saved progress, and continuous caps.
